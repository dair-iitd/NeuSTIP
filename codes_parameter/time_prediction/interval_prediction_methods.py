...
def greedy_coalescing(analysis_file_prefix,probs, thresholds, id_year_map, k=1):
    """
    params-
    ...
    return pred_min, pred_max # pred_min = torch.Size([test_data_size, top-k]) pred_min[i,j]: contains the starting interval index for ith example and top j-th score 
                              # pred_max = torch.Size([test_data_size, top-k]) pred_max[i,j]: contains the ending   interval index for ith example and top j-th score 
...
def start_end_exhaustive_sweep(start_scores, end_scores,no_grnd_start_year, no_grnd_end_year, mintime, maxtime, mean_offset, duration_matrix, use_duration, k=1):
    """
    ...
    for i in range(batch_size):
        ...
        if (use_duration == 1): #Using Gaussian Duration distribution
            sum_scores = sum_scores *duration_matrix
        ...
        if ((torch.max(start_scores[i,:]) == torch.min(start_scores[i,:])) and (torch.max(end_scores[i,:]) == torch.min(end_scores[i,:]))): # 0 groundings case
            for j, idx in enumerate(best_k):  # store start and end boundaries
            
                pred_min[i, j] = no_grnd_start_year#start
                pred_max[i, j] = no_grnd_end_year#end
        elif ((torch.max(start_scores[i,:]) == torch.min(start_scores[i,:]))): 
            for j, idx in enumerate(best_k):  
               
                end = idx % num_times
                pred_min[i, j] = max(end - mean_offset,0)
                pred_max[i, j] = end
        elif (torch.max(end_scores[i,:]) == torch.min(end_scores[i,:])): 
            for j, idx in enumerate(best_k):  
                start = torch.true_divide((idx - idx % num_times),num_times)
                pred_min[i, j] = start
                pred_max[i, j] = min(start + mean_offset,maxtime)
            ...
            for j, idx in enumerate(best_k):  # store start and end boundaries
                ...
                start = torch.true_divide((idx - end),num_times)
