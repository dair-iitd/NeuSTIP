...
def get_time_scores(scoring_function, test_kb, method='greedy-coalescing', load_to_gpu=True):
    ...
    if method in ['greedy-coalescing']:  # for time-point models
        ...
        for i in range(0, int(facts.shape[0]), 1):
            ...
            scores_t = scoring_function(s, r, o, None).data #score_t.shape: torch.Size([2, all_intervals])
            #print("scores_t: {}".format(scores_t.shape))
            #print("scores_t[-1]: {}".format(scores_t[-1].shape))
        ...
        t = torch.from_numpy(facts[:, 3:]).unsqueeze(1) # torch.Size([valid_data, 1, 6])
