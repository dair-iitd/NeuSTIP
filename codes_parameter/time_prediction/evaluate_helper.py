...
def get_gold_year_interval(t, id2_time_str, id_year_map, load_to_gpu=True):
    #print("get_gold_year_interval t: {} {}".format(t, t.shape[-1]))
    #print("len(time_index): {}".format(len(time_index)))
...
def prepare_data_iou_scores(t, test_kb, scores_t="", load_to_gpu=True): #t: true labels [3:7]
    ##
    # pdb.set_trace()
...
def load_pickle_aeiou(scores_t_dict):
    d=scores_t_dict
    ...
    print("duration_scores: {}".format(duration_scores))
...
def compute_scores(analysis_file_prefix,id_year_map, facts, t_gold_min, t_gold_max, scores_t,  no_grnd_start_year, no_grnd_end_year, mintime, maxtime, mean_offset,duration_matrix,use_duration, method=None, score_func=None,
                   thresholds=None, durations=None, topk_ranks=10, type_eval='valid'):
    
    # id_year_map.shape: torch.Size([all_years]), here torch.Size([319]) , A tensor of all the years
    # t_gold_min.shape: torch.Size([test_data_size]), here torch.Size([104])
    # t_gold_max.shape: torch.Size([test_data_size]), here torch.Size([104])
    # scores_t.shape: torch.Size([test_data_size, all_years]), here torch.Size([104, 319])
    # method: greedy-coalescing
    # thresholds.shape: torch.Size([test_data_size]), here torch.Size([104])
    # ducration = None
    ...
    if method == 'greedy-coalescing':
        ...
        probs = torch.nn.functional.softmax(scores_t, dim=-1) #probs.shape: torch.Size([test_data_size, all_years])
        pred_min, pred_max = greedy_coalescing(analysis_file_prefix,probs, thresholds, id_year_map, k=topk_ranks)
        ...
        pred_min, pred_max = start_end_exhaustive_sweep(start_probs, end_probs, no_grnd_start_year, no_grnd_end_year, mintime, maxtime, mean_offset, duration_matrix,use_duration, k=topk_ranks)
    ...
    for i in range(topk_ranks):
        # pdb.set_trace()
        iou_score = score_func(t_pred_min[:, i], t_pred_max[:, i], t_gold_min, t_gold_max, delta=0) #iou_score.shape: torch.Size([test_data_size])
...
def compute_preds(id_year_map, facts, t_gold_min, t_gold_max, scores_t, method=None, score_func=None, thresholds=None,
    ...
    if method == 'greedy-coalescing':
        # compute thresholds using val set, and do greedy coalescing
        probs = torch.nn.functional.softmax(scores_t, dim=-1)
        pred_min, pred_max = greedy_coalescing(probs, thresholds, id_year_map, k=topk_ranks)
