...
def smooth_iou_score(pred_start, pred_end, gold_start, gold_end, delta=0):
	...
	'''
	t_pred_max, t_pred_min= pred_end.unsqueeze(-1), pred_start.unsqueeze(-1)
	t_gold_max, t_gold_min= gold_end.unsqueeze(-1), gold_start.unsqueeze(-1)


	numerator   = torch.min(t_pred_max.squeeze(-1), t_gold_max.squeeze(-1)) - torch.max(t_pred_min.squeeze(-1), t_gold_min.squeeze(-1)) 
	denomerator = torch.max(t_pred_max.squeeze(-1), t_gold_max.squeeze(-1)) - torch.min(t_pred_min.squeeze(-1), t_gold_min.squeeze(-1))
	...
	if non_zero_inter.shape[0]:
		...
		score = numerator/(denomerator+1e-8).type('torch.FloatTensor')
...
def gaeiou_score(pred_start, pred_end, gold_start, gold_end, delta =0):
	...
	denomerator = denomerator.flatten()
	#--IMPORTANT!- as (t,t) interval length is counted 0---#
...
def aeiou_score(pred_start, pred_end, gold_start, gold_end, delta=0):
	...
	num_facts=len(pred_start)
	#print("Inside aeiou_score: {}".format(num_facts))
	iou_score = torch.zeros(num_facts)
	# if (num_facts == 1):
	# 	iou_score.unsqueeze()
